...
int sum_of_data(Node *head)
{

	if(!head)
		return 0;
	return ( head->data + sum_of_data(head->left) + sum_of_data(head->right) );
}

int mod_tree(Node *head)
{
	if(head)
	{
		head->sum = sum_of_data(head);
		mod_tree(head->left);
		mod_tree(head->right);
	}
}

/*void mod_tree(Node **T)
{
	if(*T==NULL)
		return;
	if((*T)->left!=NULL)
		(*T)->sum += mod_tree(&((*T)->left));
	if((*T)->right != NULL)
		(*T)->sum += mod_tree(&((*T)->right));
	(*T)->sum += (*T)->data;
